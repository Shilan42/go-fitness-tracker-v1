/*
В этом пакете содержатся функции для расчёта потраченных калорий при ходьбе, беге, а также функции для расчёта средней скорости и дистанции.
Функции этого пакета используются в других пакетах, поэтому они должны экспортируемые данного проекта.
*/
package spentenergy
...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	...
	if steps <= 0 || weight <= 0 || height <= 0 || duration <= 0 {
		return 0, errors.New("error in `WalkingSpentCalories` function: invalid calculation parameters: all values (steps, weight, height, and duration) must be positive numbers")
	}
...
func RunningSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	// Валидируем входные параметры.
	if steps <= 0 || weight <= 0 || height <= 0 || duration <= 0 {
		return 0, errors.New("error in `RunningSpentCalories` function: incorrect calculation parameters")
	}
